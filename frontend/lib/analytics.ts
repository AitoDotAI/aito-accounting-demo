/**
 * Segment analytics for the Predictive Ledger demo.
 *
 * Mirrors aito-demo's `src/analytics.js` so events from the
 * accounting demo land in the same Segment workspace as every
 * other Aito web property — the comment in that file calls out
 * "the same Segment write key as other Aito properties for
 * unified tracking."
 *
 * Differences from the source: TypeScript surface, the SURFACE
 * tag below identifies which demo emitted the event, and we
 * skip the snippet entirely when running on the dev server (no
 * point logging localhost noise).
 */
...
export function initAnalytics(): void {
  ...
  // The Segment analytics.js snippet, ported from the aito-demo
  // source one-to-one (only typing changes). Keeping the snippet
  // shape verbatim so updates to Aito's standard analytics.js
  // version can be ported without intricate diffs.
  const analytics: SegmentAnalytics = (window.analytics = window.analytics || ({ push: () => {} } as SegmentAnalytics));
